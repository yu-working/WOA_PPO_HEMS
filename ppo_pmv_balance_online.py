...
class HEMSEnvironment:
    ...
    def calculate_power(
        ...
        dehumidifier_consumption = (
            dehumidifier_power
            * max(0, (self.indoor_humidity - dehumidifier_humidity) / 100)
            if dehumidifier_on
            else 0
        )
        ...
        if ac_mode_weight[ac_mode] == 1:
            ac_consumption = (
                ...
                * (1 + 0.1 * max(0, self.indoor_temp - int(ac_temperature)))
    ...
    def step(self, action, isa):
        ...
        cooling_capacity = (
            2.8 if ac_command["ac_mode"] == 1 else 0
        )  # 暫定冷氣能力 如果為冷氣模式為 2.8 kW 送風模式為 0
        area_factor = base_area / area
        ac_temp_drop = cooling_capacity * cooling_per_kW * area_factor

        temp_final = (
            max(self.indoor_temp - ac_temp_drop, int(ac_command["ac_temp"]))
            if ac_command["ac_mode"] == 1
        ...
        rh_final = (
            max(
                self.indoor_humidity - ac_humidity_drop - dehumidifier_humidity_drop,
                dehumidifier_humidity,
            )
            if dehumidifier_humidity > 45
            else max(
                self.indoor_humidity - ac_humidity_drop - dehumidifier_humidity_drop, 45
            )
        )
...
class PPOAgent:
    ...
    def update(self, states, actions, rewards, log_probs, dones):
        ...
        surrogate_loss2 = (
            torch.clamp(prob_ratio, 1 - self.clip_range, 1 + self.clip_range)
            * advantages
        )
...
if __name__ == "__main__":
    ...
    isa = pd.read_csv(
        "C:/Users/hankli/Documents/114計劃相關/測試數據/紅外線遙控器冷氣調控指令集.csv"
    )
    ...
    for episode in range(1000):
        ...
        for step in range(24):
            ...
            # 執行動作
            next_state, reward, done, env_state, device_state, pmv, _ = env.step(
                action, isa
            )
            ...
            if step == 23:
                Device_state = Device_state.append(
                    pd.DataFrame(device_state).T, ignore_index=True
                )
                Env_state = Env_state.append(
                    pd.DataFrame(env_state).T, ignore_index=True
                )
            if done:
                break
        ...
        if episode % 100 == 0:
            ...
            print(
                f"Episode {episode}, Total Reward: {episode_reward:.2f}, Avg Energy Consumption: {avg_energy:.2f}"
            )
    ...
    Result.to_csv(
        "C:/Users/hankli/Documents/114計劃相關/調控參數/PPO能源與舒適度加權參數測試結果.csv"
    )
    end = time.time()
    print(end - start)
    # %% save & load
    ...
    for i in range(8):
        ...
        if i == 0:
            env_state_test = env_state_test.append(
                pd.DataFrame(env._get_state()[1]).T, ignore_index=True
            )

        # 執行動作
        next_state, reward, done, env_state, device_state, pmv, cost = env.step(
            action, isa
        )
        Cost.append(cost)
        Pmv.append(pmv)
        ...
        device_state_test = device_state_test.append(
            pd.DataFrame(device_state).T, ignore_index=True
        )
    ...
    Result_test["dehumidifier_hum"] = np.where(
        Result_test["dehumidifier"] == 0, "-", Result_test["dehumidifier_hum"]
    )
    Result_test.to_csv(
        "C:/Users/hankli/Documents/114計劃相關/調控參數/PPO能源與舒適度加權參數應用結果.csv"
    )
    end = time.time()
    print(end - start)
    ...
    # 計算節能效果
    data = pd.read_csv(
        "C:/Users/hankli/Documents/114計劃相關/測試數據/nilm_data_ritaluetb_hour.csv"
    )
