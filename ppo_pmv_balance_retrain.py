...
class PPOAgent:
    ...
    def save_model(self, room_id):
        ...
        torch.save(
            ...
            "./config/ppo_pmv_balance_" + room_id + ".pt",
        )
...
def ppo_retrain(indoor_data, room_id):
    ...
    for episode in range(len(indoor_data)):
        state = env.reset(
            indoor_temp=indoor_data[episode][0], indoor_humidity=indoor_data[episode][1]
        )
        episode_reward = 0
        ...
        for step in range(24):
            ...
            if step == 23:
                Device_state = pd.concat(
                    [Device_state, pd.DataFrame(device_state).T], ignore_index=True
                )
                Env_state = pd.concat(
                    [Env_state, pd.DataFrame(env_state).T], ignore_index=True
                )
            if done:
                break
    ...
    print(end - start)
    # %% save & load
    agent.save_model(room_id=room_id)
