...
def calculate_power(
    ...
    if dehumidifier_on == "on":
        dehumidifier_consumption = dehumidifier_power * max(
            0, (indoor_humidity - dehumidifier_humidity) / 100
        )
    else:
        dehumidifier_consumption = 0
...
def pmv_balance(data):
    try:
        ...
        for category, df_list in Device.items():
            ...
            Device[category] = {
                name: df for name, df in zip(names, df_list)
            }  # 將電器參數名稱也賦予上去

        # %% 實際資料
        logger.info("Phase2: 電器資料萃取")
        try:
            ac_parameter = pd.concat(
                [
                    Device["air_conditioner"]["cfg_fan_level"][
                        "record_value"
                    ].reset_index(drop=True),
                    Device["air_conditioner"]["cfg_mode"]["record_value"].reset_index(
                        drop=True
                    ),
                    Device["air_conditioner"]["cfg_temperature"][
                        "record_value"
                    ].reset_index(drop=True),
                    Device["air_conditioner"]["cfg_temperature"][
                        "device_signature"
            ...
            ac_parameter.columns = [
                "cfg_fan_level",
                "cfg_mode",
                "cfg_temperature",
                "device_signature",
            ]  # 欄位再命名
            operation_parameter = pd.concat(
                [
                    Device["air_conditioner"]["cfg_temperature"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["air_conditioner"]["cfg_mode"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["air_conditioner"]["cfg_fan_level"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["dehumidifier"]["cfg_power"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["dehumidifier"]["cfg_humidity"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["ac_outlet"]["cfg_power"]["operation_direction"].reset_index(
                        drop=True
                    ),
                ]
            )
            user_source = pd.concat(
                [
                    Device["air_conditioner"]["cfg_temperature"]["source"].reset_index(
                        drop=True
                    ),
                    Device["air_conditioner"]["cfg_mode"]["source"].reset_index(
                        drop=True
                    ),
                    Device["air_conditioner"]["cfg_fan_level"]["source"].reset_index(
                        drop=True
                    ),
                    Device["dehumidifier"]["cfg_power"]["source"].reset_index(
                        drop=True
                    ),
                    Device["dehumidifier"]["cfg_humidity"]["source"].reset_index(
                        drop=True
                    ),
            ...
            remote_parameter = pd.concat(
                [
                    Device["remote"]["cfg_fan_level"]["record_value"].reset_index(
                        drop=True
                    ),
                    Device["remote"]["cfg_mode"]["record_value"].reset_index(drop=True),
                    Device["remote"]["cfg_temperature"]["record_value"].reset_index(
                        drop=True
                    ),
                    Device["remote"]["cfg_temperature"]["device_signature"].reset_index(
                        drop=True
                    ),
                ],
                axis=1,
            )  # 紅外線遙控器冷氣參數取出
            remote_parameter.columns = [
                "cfg_fan_level",
                "cfg_mode",
                "cfg_temperature",
                "device_signature",
            ...
            operation_parameter = pd.concat(
                [
                    Device["remote"]["cfg_temperature"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["remote"]["cfg_mode"]["operation_direction"].reset_index(
                        drop=True
                    ),
                    Device["remote"]["cfg_fan_level"][
                        "operation_direction"
                    ].reset_index(drop=True),
                    Device["dehumidifier"]["cfg_power"][
                        "operation_direction"
            ...
            user_source = pd.concat(
                [
                    Device["remote"]["cfg_temperature"]["source"].reset_index(
                        drop=True
                    ),
        ...
        dehumidifier_parameter = pd.concat(
            [
                Device["dehumidifier"]["cfg_humidity"]["record_value"].reset_index(
                    drop=True
                ),
                Device["dehumidifier"]["cfg_power"]["record_value"].reset_index(
                    drop=True
                ),
                Device["dehumidifier"]["cfg_power"]["device_signature"].reset_index(
                    drop=True
                ),
            ],
            axis=1,
        )  # 除濕機參數取出
        dehumidifier_parameter.columns = [
            "cfg_humidity",
            "cfg_power",
            "device_signature",
        ]
        fan_parameter = pd.DataFrame(
            Device["ac_outlet"]["cfg_power"]["record_value"].reset_index(drop=True)
        )  # 風扇參數取出
        fan_parameter = pd.concat(
            [
                Device["ac_outlet"]["cfg_power"]["record_value"].reset_index(drop=True),
                Device["ac_outlet"]["cfg_power"]["device_signature"].reset_index(
                    drop=True
                ),
            ],
            axis=1,
        ...
        if (
            ac_parameter["cfg_fan_level"][0] != "low"
            and ac_parameter["cfg_fan_level"][0] != "auto"
        ):
            ac_parameter["cfg_fan_level"][0] = "high"
        # 移除異常值
        Device["sensor"]["op_humidity"] = Device["sensor"]["op_humidity"][
            ~(
                (Device["sensor"]["op_humidity"]["record_value"] == "unknown")
                | (Device["sensor"]["op_humidity"]["record_value"] == "unavailable")
            )
        ]
        Device["sensor"]["op_temperature"] = Device["sensor"]["op_temperature"][
            ~(
                (Device["sensor"]["op_temperature"]["record_value"] == "unknown")
                | (Device["sensor"]["op_temperature"]["record_value"] == "unavailable")
            )
        ]
        # %% 初始用電量計算
        logger.info("Phase3: 初始用電量計算")
        indoor_data = np.array(
            [
                (Device["sensor"]["op_temperature"]["record_value"].astype(float)),
                Device["sensor"]["op_humidity"]["record_value"].astype(float),
            ]
        ).T  # IoT溫溼度擷取
        try:
            cost_init = calculate_power(
                dehumidifier_humidity=float(dehumidifier_parameter["cfg_humidity"][0]),
                indoor_humidity=indoor_data[0][1],
                dehumidifier_on=dehumidifier_parameter["cfg_power"][0],
                fan_on=fan_parameter["cfg_power"][0],
                ac_temperature=float(ac_parameter["cfg_temperature"][0]),
                indoor_temp=indoor_data[0][0],
                ac_mode=ac_parameter["cfg_mode"][0],
            ...
            cost_init = calculate_power(
                ...
                dehumidifier_on=dehumidifier_parameter["cfg_power"][0],
                fan_on=fan_parameter["cfg_power"][0],
                ac_temperature=float(remote_parameter["cfg_temperature"][0]),
                indoor_temp=indoor_data[0][0],
                ac_mode=remote_parameter["cfg_mode"][0],
                ac_fan_speed=remote_parameter["cfg_fan_level"][0],
            )

        # %% PPO再訓練 -- 若過往24小時有手動操作過抑或初次使用抓不到對應room_id的ppo模型時進行
        if (
            "write" in operation_parameter.values and "user" in user_source.values
        ) or os.path.isfile(
            "./config/ppo_pmv_balance_" + room_id[0][0] + ".pt"
        ) == False:
        ...
        # device_state_init = []
        state = Env.reset(
            test=True, indoor_temp=indoor_data[0][0], indoor_humidity=indoor_data[0][1]
        )
        # %%
        ...
        while (
            ...
            Result_woa = pd.concat([Result_woa, result], ignore_index=True)
            # 處理結果數據
            Pmv_woa = pd.DataFrame([pmv_woa], columns=["pmv"])
            Result_woa = pd.concat([Result_woa, Pmv_woa], axis=1)
            Result_woa["ac_temp"] = np.where(
                Result_woa["ac_mode"] == 0, "0", Result_woa["ac_temp"]
            )
            Result_woa["ac_temp"] = np.where(
                Result_woa["ac_mode"] == 2, "0", Result_woa["ac_temp"]
            )
            Result_woa["dehumidifier_hum"] = Result_woa["dehumidifier_hum"].astype(int)
            Result_woa["dehumidifier_hum"] = (
                round(Result_woa["dehumidifier_hum"] / 5) * 5
            )
            Result_woa["dehumidifier_hum"] = np.where(
                Result_woa["dehumidifier"] == 0, "0", Result_woa["dehumidifier_hum"]
            )
            ...
            # 執行動作
            next_state, reward, done, env_state, device_state, pmv_ppo, cost_ppo = (
                Env.step(action, isa)
            )
            Cost_ppo += cost_ppo
            env_state_test = pd.concat(
                [env_state_test, pd.DataFrame(env_state).T], ignore_index=True
            )
            device_state_test = pd.concat(
                [device_state_test, pd.DataFrame(device_state).T], ignore_index=True
            )
            ...
            Result_ppo["dehumidifier_hum"] = (
                round(Result_ppo["dehumidifier_hum"] / 5) * 5
            )
            Result_ppo["dehumidifier_hum"] = np.where(
                Result_ppo["dehumidifier"] == 0, "0", Result_ppo["dehumidifier_hum"]
            )
        ...
        feedback_system.record_feedback(
            room_id[0][0], current_state, system_state, user_state
        )
        # 預測使用者偏好（之後可使用）
        predicted = feedback_system.predict_user_preference(
            current_state["current_temp"], current_state["current_humidity"]
        )

        # %% 數值轉換
        logger.info("Phase6: 數值轉換")
        Result["dehumidifier"] = np.where(
            (Result["dehumidifier"] == 0) | (Result["dehumidifier"] == "0"),
            "off",
            Result["dehumidifier"],
        )
        Result["dehumidifier"] = np.where(
            (Result["dehumidifier"] == 1) | (Result["dehumidifier"] == "1"),
            "on",
            Result["dehumidifier"],
        )
        Result["fan_state"] = np.where(
            (Result["fan_state"] == 0) | (Result["fan_state"] == "0"),
            "off",
            Result["fan_state"],
        )
        Result["fan_state"] = np.where(
            (Result["fan_state"] == 1) | (Result["fan_state"] == "1"),
            "on",
            Result["fan_state"],
        )
        Result["ac_mode"] = np.where(
            (Result["ac_mode"] == 1) | (Result["ac_mode"] == "1"),
            "cool",
            Result["ac_mode"],
        )
        Result["ac_mode"] = np.where(
            (Result["ac_mode"] == 0) | (Result["ac_mode"] == "0"),
            "fan",
            Result["ac_mode"],
        )
        Result["ac_fan"] = np.where(
            (Result["ac_fan"] == 0) | (Result["ac_fan"] == "0"), "low", Result["ac_fan"]
        )
        Result["ac_fan"] = np.where(
            (Result["ac_fan"] == 1) | (Result["ac_fan"] == "1"),
            "high",
            Result["ac_fan"],
        )
        Result["ac_fan"] = np.where(
            (Result["ac_fan"] == 2) | (Result["ac_fan"] == "2"),
            "auto",
            Result["ac_fan"],
        )

        try:
            Result["ac_temp"] = np.where(
                Result["ac_temp"] == "-", "0", Result["ac_temp"]
            )
        except:
            pass
        try:
            Result = Result.drop(
                ["env_temp", "env_humd", "best_temp", "best_humd"], axis=1
            )
        except:
        ...
        # 使用者偏好數值轉換
        try:
            predicted["dehumidifier"] = str(predicted["dehumidifier"]).replace(
                "0", "off"
            )
            predicted["dehumidifier"] = str(predicted["dehumidifier"]).replace(
                "1", "on"
    ...
    # %% 例外紀錄
        ...
        logging.error(
            "Error Line {}".format(sys.exc_info()[-1].tb_lineno), type(e).__name__, e
        )
