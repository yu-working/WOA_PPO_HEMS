...
class WhaleOptimizationHEMS:
    ...
    def calculate_power(self, dehumidifier_humidity, indoor_humidity,
        ...
        # 計算總功耗
        dehumidifier_power = 120  # 除濕機功率(kW)
        fan_power = 60             # 電扇功率(kW)
        ac_base_power =  1350        # 冷氣基礎功率(kW)
    ...
    def fitness_function(self, temp: float, humidity: float, new_temp, new_humidity, step, trees) -> float:
        """
        計算適應度值(基於舒適度和能源消耗)
        ...
        if float(self.pmv_down) < pmv_value < float(self.pmv_up):
            pass
        else: fitness += 100000000  # 當PMV值不在舒適範圍內時增加懲罰值
        return fitness
    ...
    def optimize(self, indoor_data, steps, trees) -> Tuple[np.ndarray, float, List[float]]:
        """
        執行鯨魚優化演算法
        ...
        fitness_values = np.array([
            self.fitness_function(pos[0], pos[1], pos[0], pos[1], steps, trees) 
...
#%%
    ...
    for i in range(14, 21):
        ...
        best_position, best_fitness, fitness_history, device_state, cost, pmv = woa.optimize(indoor_data, i, trees)
        Pmv.append(pmv)
        Cost.append(cost)
        ...
        if i in [0, 1, 2, 3, 4, 5, 6, 7, 8]:
            price = 1.96
        elif i in [9, 10, 11, 12, 13, 14, 15, 22, 23]:
            price = 4.54
        else:
            price = 6.92
        ec_cost = ec/1000*price
        esr = (1-(cost/ec_cost))*100
        Esr.append(esr)
    ...
    print(Result)
    Result.to_csv('./WOA_TREE用電分配最佳化測試結果.csv')